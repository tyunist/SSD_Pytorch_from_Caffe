...
class CaffeData(nn.Module):
    def __init__(self, layer):
        ...
        caffe.set_mode_cpu()
        if dict_has_key(layer, 'include') and layer['include'] == 'TRAIN':
...
class CaffeNet(nn.Module):
    ...
    def load_weights(self, caffemodel):
        if self.has_mean:
            ...
            if dict_has_key(self.net_info['props'], 'input_shape'):
        ...
        while i < layer_num:
            ...
            if dict_has_key(layer, 'include') and dict_has_key(layer['include'], 'phase'):
                ...
                lname = lname + '_' + phase
            ltype = layer['type']
            if not dict_has_key(lmap, lname):
                i = i + 1
                continue
