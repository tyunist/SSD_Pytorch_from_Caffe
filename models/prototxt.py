...
def parse_prototxt(protofile):
    ...
    def parse_block(fp):
        ...
        while line != '}':
            ...
            if ltype == 0: # key: value
                ...
                try:
                    _has_key = block.has_key(key)
                except:
                    _has_key = key in block 
    ...
    while line != '':
        ...
        if ltype == 0: # key: value
            ...
            value = value.strip().strip('"')
            try:
                _has_key = props.has_key(key)
            except:
                _has_key = key in props
...
def save_prototxt(net_info, protofile, region=True):
    ...
    try:
        _has_key = props.has_key('input')
    except:
        _has_key = 'input' in props

    if _has_key:
        print >> fp, 'input: \"%s\"' % props['input']
    try:
        _has_key = props.has_key('input_dim')
    except:
        _has_key = 'input_dim' in props
