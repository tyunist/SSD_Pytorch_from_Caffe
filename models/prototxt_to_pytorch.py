...
class CaffeNet(nn.Module):
    def __init__(self, protofile, width=None, height=None, channels=None, omit_data_layer=False, phase='TRAIN'):
        ...
        self.has_mean = False
        if dict_has_key(self.net_info['props'], 'mean_file'):
            self.has_mean = True
        ...
        self.is_detection_output = False
    ...
    def set_detection_output(self, is_detection_output):
        self.is_detection_output = is_detection_output 
    ...
    def forward(self, *inputs): 
        ...
        while i < layer_num:
            layer = layers[i]
            lname = layer['name']
            if dict_has_key(layer, 'include') and dict_has_key(layer['include'],'phase'):
                phase = layer['include']['phase']
                #Ty: Replace '.' by '_' due to the incompatability of new pytorch versions
                #lname = lname + '.' + phase
                lname = lname + '_' + phase
                if phase != self.phase:
                    i = i + 1
            ...
            # Skip detection output if necessary since it is slow
            if ltype == 'DetectionOutput':
                if not self.is_detection_output:
                    print('Ignore %-30s' % (lname))
                    i = i + 1 
                    continue
            ...
            if True:
                ...
                if ltype in ['SoftmaxWithLoss', 'MultiBoxLoss']:
                    if self.output_loss != None:
                        self.output_loss.update(tdatas[0])
                    else:
            ...
            try:
                ...
                output_size = [type(self.blobs[tnames[0]]), len(self.blobs[tnames[0]])]
                #output_size = self.blobs[tnames[0]].size()
            if self.verbose:
    ...
    def get_loss(self, return_format='list'):
        if return_format == 'dict':
            return self.output_loss 

        total_loss = self.output_loss['total_loss'] 
        loss_l     = self.output_loss['loss_l'] 
        loss_c     = self.output_loss['loss_c'] 
    ...
    def create_network(self, net_info, input_width = None, input_height = None, input_channels = None):
        ...
        if dict_has_key(props, 'input_shape'):
        ...
        elif dict_has_key(props, 'input_dim'):
        ...
        while i < layer_num:
            ...
            if ltype == 'Convolution':
                ...
                stride = int(convolution_param['stride']) if dict_has_key(convolution_param, 'stride') else 1
                pad = int(convolution_param['pad']) if dict_has_key(convolution_param, 'pad') else 0
                group = int(convolution_param['group']) if dict_has_key(convolution_param, 'group') else 1
                dilation = 1
                if dict_has_key(convolution_param, 'dilation'):
                    dilation = int(convolution_param['dilation'])
                bias = True
                if dict_has_key(convolution_param, 'bias_term') and convolution_param['bias_term'] == 'false':
                    bias = False
                ...
                momentum = 0.9
                if dict_has_key(layer, 'batch_norm_param') and dict_has_key(layer['batch_norm_param'], 'moving_average_fraction'):
                ...
                inplace = (bname == tname)
                if dict_has_key(layer, 'relu_param') and dict_has_key(layer['relu_param'], 'negative_slope'):
                ...
                padding = 0
                if dict_has_key(layer['pooling_param'], 'pad'):
                ...
                operation = 'SUM'
                if dict_has_key(layer, 'eltwise_param') and dict_has_key(layer['eltwise_param'], 'operation'):
                ...
                blob_height[tname] = blob_height[bname]
                i = i + 1
            #Ty: implement MyNormalize, a custom layer in ASU model
            elif ltype == 'MyNormalize':
                channels = blob_channels[bname]
                scale = float(layer['my_norm_param']['scale_filler']['value'])
                channel_shared = (layer['my_norm_param']['channel_shared'] == 'true')
                models[lname]  = MyNormalize(channels, scale, channel_shared)
                blob_channels[tname] = blob_channels[bname]
                blob_width[tname] = blob_width[bname]
                blob_height[tname] = blob_height[bname]
                i = i + 1
                ...
                axis = 1
                if dict_has_key(layer, 'concat_param') and dict_has_key(layer['concat_param'], 'axis'):
                ...
                max_size = -1
                if dict_has_key(layer['prior_box_param'], 'max_size'):
                    max_size = float(layer['prior_box_param']['max_size'])
                aspects = []
                if dict_has_key(layer['prior_box_param'], 'aspect_ratio'):
                ...
                flip = False
                if dict_has_key(layer['prior_box_param'], 'flip'):
                ...
                axis = 1
                if dict_has_key(layer, 'softmax_param') and dict_has_key(layer['softmax_param'], 'axis'):
